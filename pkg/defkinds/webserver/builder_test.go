...
func initBuildLLBWithCacheMountsTC(t *testing.T, mockCtrl *gomock.Controller) buildTC {
	genericDef := loadGenericDef(t, "testdata/build/zbuild.yml")
	genericDef.RawLocks = loadDefLocks(t, "testdata/build/zbuild.lock")
	...
	return buildTC{
		...
		expectedState: "testdata/build/state-with-cache-mounts.json",
		expectedImage: &image.Image{
			Image: specs.Image{
				Architecture: "amd64",
				OS:           "linux",
...
func TestBuild(t *testing.T) {
	testcases := map[string]func(*testing.T, *gomock.Controller) buildTC{
		...
		"build LLB with cache mounts":                                initBuildLLBWithCacheMountsTC,
